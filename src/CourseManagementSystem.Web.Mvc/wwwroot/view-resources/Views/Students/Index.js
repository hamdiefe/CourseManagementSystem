    ...
    $(function () {
        ...
        var datatable = $.HSCore.components.HSDatatables.init($('#datatable'), {
            ...
            ajax: function (data, callback, settings) {
                ...
                abp.ui.setBusy(_$studentsTable);
                _studentsService.getAll(filter).done(function (result) {
                    callback({
                    ...
                    abp.ui.clearBusy(_$studentsTable);
                });
            },
            columnDefs: [
                {
                ...
                },
                {
                    className: 'clickView',
                    targets: 11,
                    data: 'student.hourlyPaymentPeriod',
                ...
                },     
                {
                    ...
                    render: (data, type, row, meta) => {
                        return row.student.hourlyPaymentAmount + " " + l("Enum_Currency_" + row.student.currency);
                    }
                },
        ...
        $('#datatable tbody').on('click', '.clickView', function () {
            ...
            if (data) {
                $("#EditStudentButton").trigger("click");
                createOrEdit(data)
            }
        });
        ...
        $("#DeleteSelectedAllButton").click(function myfunction() {
            ...
            for (var i = 0; i < selectedRows.length; i++) {
                students.push(selectedRows[i].student.id)
            }

            deleteSelectedAllStudents(students);
        });     

        function createOrEdit(data) {
            var url = abp.appPath + 'Students/CreateOrEditModal/';

            if (data) {
                url = url + data.student.id
            }
        ...
        function deleteSelectedAllStudents(data) {
            abp.message.confirm(
                abp.utils.formatString(
        ...
        function viewStudent(data) {
            abp.ajax({
                url: abp.appPath + 'Students/ViewModal/' + data.student.id,
                type: 'POST',
                dataType: 'html',
                success: function (content) {
        ...
        abp.event.on('student.createOrEditStudentModalSaved', function () {
            getStudents();
        });
        ...
        $('#toggleColumn_school').change(function (e) {
            datatable.columns(8).visible(e.target.checked)
        });

        $('#toggleColumn_grade').change(function (e) {
            datatable.columns(9).visible(e.target.checked)
        });

        $('#toggleColumn_studentNumber').change(function (e) {
            datatable.columns(10).visible(e.target.checked)
        });

        $('#toggleColumn_hourlyPaymentPeriod').change(function (e) {
            datatable.columns(11).visible(e.target.checked)
        });

        $('#toggleColumn_hourlyPaymentAmount').change(function (e) {
        ...
        $('#toggleColumn_phone').change(function (e) {
            datatable.columns(14).visible(e.target.checked)
        });

        $('#toggleColumn_address').change(function (e) {
            datatable.columns(15).visible(e.target.checked)
        });
